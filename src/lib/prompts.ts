...
export const getArticlePrompt = ({
  ...
  return `
        ...
        - 대상 독자: 한국어 사용자
        - **콘텐츠 분량 (매우 중요)**: SEO 점수 100점을 위해, 전체 글자 수는 반드시 **1,800 단어에서 2,200 단어 사이**여야 합니다. 이는 100점 달성을 위한 필수 조건입니다. 각 섹션을 매우 상세하고 깊이 있게 다루어 분량을 확보하고, 특히 새로 추가된 '실패 사례와 극복 방법' 섹션을 포함하여 모든 소제목에 충실한 내용을 채워 이 분량을 달성해주세요.
        - **키워드 밀도 (가장 중요한 규칙)**: SEO 100점 달성을 위해, 핵심 키워드 '${keyword}'의 밀도를 **정확히 1.5% ~ 2.5%**로 맞춰야 합니다. 이는 **가장 중요한 요구사항**입니다. 예를 들어, 총 단어가 2,000개라면 '${keyword}'는 30번에서 50번 사이로 등장해야 합니다. 글을 완성한 후, 직접 단어 수와 키워드 등장 횟수를 세어 밀도를 확인하고, 범위를 벗어난다면 반드시 수정하여 이 규칙을 준수해야 합니다.
        ...
        - **키워드 강조**: 본문의 각 H2 섹션 내에서, 핵심 키워드 '${keyword}'를 1~2회 <strong>${keyword}</strong> 와 같이 <strong> 태그를 사용하여 굵게 표시해주세요. 이는 독자의 시선을 끌고 키워드의 중요성을 강조하는 데 도움이 됩니다.
