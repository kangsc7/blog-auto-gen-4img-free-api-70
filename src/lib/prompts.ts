...
export const getArticlePrompt = ({
  ...
  return `
        ...
        - 대상 독자: 한국어 사용자
        - **콘텐츠 분량 (매우 중요)**: SEO 점수 100점을 위해, 전체 글자 수는 반드시 **2,000 단어에서 2,500 단어 사이**여야 합니다. 이 분량은 AI 모델의 최대 출력 길이를 고려한 것입니다. 각 섹션을 매우 상세하고 깊이 있게 다루어 분량을 확보하고, 특히 새로 추가된 '실패 사례와 극복 방법' 섹션을 포함하여 모든 소제목에 충실한 내용을 채워 이 분량을 달성해주세요.
