...
export const getArticlePrompt = ({
  ...
  return `
        ...
        - 대상 독자: 한국어 사용자
        - **콘텐츠 분량 (매우 중요)**: SEO 점수 100점을 위해, 전체 글자 수는 반드시 **2,500 단어에서 3,000 단어 사이**여야 합니다. 이 규칙은 절대적으로 지켜져야 합니다. 각 섹션을 매우 상세하고 깊이 있게 다루어 분량을 확보하세요.
        - **키워드 밀도 (매우 중요)**: SEO 100점을 위해, 핵심 키워드 '${keyword}'가 전체 텍스트에서 차지하는 비율을 **정확히 1.5%에서 2.5% 사이**로 유지해야 합니다. 이 규칙 또한 절대적으로 중요합니다. 키워드 밀도 계산법은 '(키워드 총 등장 횟수 / 전체 단어 수) * 100' 입니다.
        ...
        - **가장 중요한 최종 규칙**: 위에서 **(매우 중요)**라고 표시된 **콘텐츠 분량**과 **키워드 밀도** 지침은 이 작업에서 가장 중요합니다. 어떤 경우에도 이 두 가지 규칙을 어겨서는 안 됩니다.
...
```typescript
import { getCssStyles } from './htmlTemplate';
