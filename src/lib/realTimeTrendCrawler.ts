...
export class RealTimeTrendCrawler {
  private static async crawlNaverTrends(apiKey: string): Promise<string[]> {
    try {
      ...
      const prompt = `현재 시각: ${currentYear}년 ${currentMonth}월 ${timeContext} ${currentHour}시

${currentYear}년 한국의 실시간 검색 트렌드를 분석하여 지금 이 순간 실제로 검색되고 있을 만한 키워드 10개를 생성해주세요.
...
**포함해야 할 분야 (${currentYear}년 기준):**
- 경제/금융: ${currentYear}년 새로운 정책, 지원금, 금리 변화
- 생활/복지: ${currentYear}년 건강보험, 육아지원, 주거정책
- 기술/IT: ${currentYear}년 새로운 앱, 서비스 출시
- 건강/의료: ${currentYear}년 계절별 건강 정보, 의료 혜택
- 사회/문화: ${currentYear}년 축제, 이벤트, 생활 변화
- 날씨/계절: ${currentYear}년 ${currentMonth}월 현재 계절에 맞는 정보
...
각 키워드는 15자 이내로 하고, ${currentYear}년 현재 실제 검색할 만한 구체적인 내용으로 만들어주세요.
한 줄에 하나씩, 키워드만 나열해주세요.`;
  ...
  private static async crawlGoogleTrends(apiKey: string): Promise<string[]> {
    try {
      ...
      const prompt = `현재 날짜: ${currentYear}년 ${currentMonth}월 ${currentDate.getDate()}일

구글 트렌드에서 ${currentYear}년 현재 급상승하고 있을 만한 한국 관련 검색어 10개를 생성해주세요.

**계절 특성 반영 (${currentYear}년 ${season}):**
${season === '봄' ? '- 새학기, 꽃구경, 봄나들이, 알레르기 관리' :
  season === '여름' ? '- 휴가, 더위 대책, 여름 건강관리, 에어컨' :
  season === '가을' ? '- 가을 여행, 건조함 대책, 환절기 건강' :
  '- 겨울 건강, 난방비 절약, 연말정산, 새해 계획'}

**트렌드 특성 (${currentYear}년):**
- ${currentYear}년 최근 1주일 내 이슈
- 실용적이고 검색 가치가 있는 내용
- 정치/연예 스캔들 제외
- 생활밀착형 정보

각 키워드는 구체적이고 ${currentYear}년 현재 실제 검색할 만한 내용으로, 한 줄에 하나씩 나열해주세요.`;
...
특징:
- 15자 이내
- ${currentYear}년 실용적이고 검색 가치가 있는 내용
- 계절과 시기에 맞는 내용
- 다른 설명 없이 키워드만 제공

예시 형태: "${currentYear}년 겨울 실내 습도 조절법", "${currentYear}년 연말정산 절세 팁", "${currentYear}년 신년 다이어트 계획"`;
