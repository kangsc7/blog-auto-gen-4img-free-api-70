...
1. **년도 정보 완전 보존**: 
   - 키워드에 포함된 "${extractedYear}년" 정보는 절대로 누락하면 안 됩니다
   - 모든 제목은 반드시 "${extractedYear}년"으로 시작해야 합니다
   - "년 디지털플랫폼정부법" 같은 잘못된 형태는 절대 금지입니다
   - 올바른 형태: "${extractedYear}년 �지털플랫폼정부법..."
   
2. **제목 구조 엄격 준수**:
   - 첫 번째 단어는 반드시 "${extractedYear}년"이어야 합니다
   - 그 다음에 바로 핵심 키워드가 와야 합니다
   - 예시: "${extractedYear}년 �지털플랫폼정부법 시행에 따른 국민디지털지원금..."
   
3. **키워드 완전성 보장**:
   - 핵심 키워드 '${keyword}'의 모든 구성 요소를 절대 누락하지 마세요
   - 각 제목에는 반드시 핵심 키워드가 완전한 형태로 포함되어야 합니다

**제목 생성 지침**:
- 각 제목은 "${extractedYear}년"으로 반드시 시작하세요
- 모든 제목은 반드시 한글로만 작성해야 하며, 한자나 다른 언어는 절대 포함하지 마세요
- ${currentYear}년 최신 상황을 반영한 시의성 있는 제목으로 작성해주세요
- 결과는 각 제목을 줄바꿈으로 구분하여 번호 없이 텍스트만 제공해주세요
- 다른 설명 없이 주제 목록만 생성해주세요

**최종 검증 사항**:
1. 모든 제목이 "${extractedYear}년"으로 시작하는가?
2. '${keyword}'의 모든 단어가 제목에 포함되었는가?
3. "년 " (년 다음에 공백) 형태로 잘못 사용되지 않았는가?

⚠️ 중요: 제목은 반드시 "${extractedYear}년 ..." 형태로 시작해야 하며, "년"이 단독으로 사용되면 안 됩니다.
