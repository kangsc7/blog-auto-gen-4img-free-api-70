...
export const getEnhancedArticlePrompt = async ({
  ...
  return `
        ...
        **전체 글자수는 공백 포함 4,350자를 절대 초과해서는 안 됩니다.**
        ...
        - 각 H2 섹션의 본문은 **190자에서 250자 사이**로 작성
...
export const generateBlogArticlePrompt = async (
  ...
  // 동적 소제목 생성 (5개로 축소) - 40자 제한 적용
  console.log('동적 소제목 생성 시작 (40자 제한):', keyword, topic);
  const dynamicHeadings = await generateDynamicHeadings(keyword, topic, 'default');
  console.log('생성된 동적 소제목 (40자 제한):', dynamicHeadings.map(h => `${h.title} (${h.title.length}자)`));
  ...
  // 웹 크롤링으로 최신 정보 및 공식 링크 수집
  const crawledInfo = await WebCrawlerService.crawlForKeyword(keyword, 'default');
...
--- HTML TEMPLATE END ---
  ...
  if (referenceLink || referenceSentence) {
    ...
    if (referenceLink) {
      prompt += `**참조 링크**: ${referenceLink}\n`;
      prompt += `이 링크의 내용을 분석하여 글에 관련 정보를 자연스럽게 포함시키세요.\n`;
    }
    
    if (referenceSentence) {
      prompt += `**참조 문장**: "${referenceSentence}"\n`;
      prompt += `이 문장과 관련된 내용을 글에 포함하여 맥락을 풍부하게 만드세요.\n`;
    }
  }
  ...
  if (officialLink) {
    prompt += `\n## 🔗 공식 링크 포함 지침\n`;
    prompt += `**공식 링크**: ${officialLink}\n`;
    prompt += `이 공식 링크를 글 내용에 자연스럽게 하이퍼링크로 포함시키세요. 예: [관련 공식 정보 더 보기](${officialLink})\n`;
  }
