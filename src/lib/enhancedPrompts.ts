...
const PROTECTED_GUIDELINES = {
  WORD_COUNT_LIMIT: "각 H2 섹션 300-500자",
...
const getRandomPerspective = () => {
  const perspectives = [
    {
      name: '개인 경험 기반',
      description: '실제 경험담과 후기를 중심으로 구성',
      style: '친근하고 진솔한 톤으로 개인적 경험을 공유하는 방식'
    },
    {
      name: '질문과 답변 구조',
      description: '독자가 궁금해할 질문을 먼저 제시하고 답변하는 형식',
      style: '대화형 구조로 독자의 궁금증을 해결해주는 방식'
    },
    {
      name: '잘못된 상식 반박',
      description: '일반적인 오해나 잘못된 정보를 바로잡는 관점',
      style: '사실 확인과 정정을 통해 올바른 정보를 제공하는 방식'
    },
    {
      name: '전문가 조언',
      description: '전문가나 의사의 관점에서 설명하는 방식',
      style: '전문적이면서도 이해하기 쉽게 설명하는 권위적 톤'
    },
    {
      name: '스토리텔링 중심',
      description: '비유와 사례, 이야기를 통해 전달하는 방식',
      style: '흥미로운 사례와 비유로 쉽게 이해시키는 서사적 구조'
    },
    {
      name: '가이드북 형식',
      description: '단계별 가이드와 체크리스트 중심 구성',
      style: '실행 가능한 단계와 팁을 체계적으로 제시하는 방식'
    }
  ];
...
const getRandomWritingStyle = () => {
  const styles = [
    { name: '친근한 대화체', pattern: '~해요, ~죠, ~네요' },
    { name: '정중한 설명체', pattern: '~습니다, ~됩니다' },
    { name: '친밀한 조언체', pattern: '~하시면 좋을 것 같아요, ~해보세요' },
    { name: '경험 공유체', pattern: '~했어요, ~더라고요, ~하더군요' },
    { name: '질문 유도체', pattern: '~하신가요?, ~해보신 적 있나요?' },
    { name: '격려 응원체', pattern: '~할 수 있어요, ~해내실 거예요' }
  ];
...
// 대체 표현 생성
const generateAlternativeExpressions = (keyword: string): string => {
  const expressions = [
    `${keyword}의 새로운 접근법`,
    `${keyword} 완전 정복`,
    `${keyword} 실전 가이드`,
...
// 다양한 예시 유형 생성
const generateVariedExamples = (topic: string, keyword: string) => {
  const exampleTypes = [
    {
      type: '실제 사례 중심',
      examples: ['성공 사례', '실패 경험담', '개선 사례']
    },
    {
      type: '통계 데이터 기반',
      examples: ['최신 통계', '연구 결과', '설문 조사']
    },
    {
      type: '비교 분석',
      examples: ['전후 비교', '유형별 비교', '방법별 차이점']
    },
    {
      type: '단계별 가이드',
      examples: ['초보자 단계', '중급자 팁', '고급 활용법']
    }
  ];
  
  return exampleTypes[Math.floor(Math.random() * exampleTypes.length)];
};
...
3. **검색 트렌드 고려**: 최신 검색 트렌드와 사용자 관심사를 반영한 소제목과 함께, 각 섹션별 약 300-500자의 풍부한 설명을 작성해주세요.
4. 설명은 단순 요약이 아닌, 다음 요소를 반드시 포함해야 합니다:
   - 사례 또는 통계 기반 인용
   - 독자 입장에서 느끼는 감정 또는 공감 지점
   - 전문가 또는 실사용자의 조언 또는 팁
   - 블로그 방문자가 '찾고 싶던 정보가 여기 다 있다'고 느낄 만큼 깊이 있고 실용적이어야 합니다
   - 검색 목적에 부합하는 완결형 콘텐츠처럼 느껴지도록 구성해주세요
   - 각 섹션은 공백 포함 300자에서 500자 사이로 작성해주세요
   - 설명이 140자를 초과하면 두 번째 마침표(.) 뒤에서 반드시 </p> 태그 닫고 공백 줄바꿈 추가 후 새로운 <p> 태그 시작
   - 각 문단은 <p> 태그로 감싸고, <p> 태그 사이에는 줄바꿈을 추가해야 합니다
   - 소제목과 본문은 모두 왼쪽 정렬(text-align: left)을 적용해야 합니다
...
const getHtmlTemplate = (topic: string, content: string, ...args: string[]): string => {
  return `<h3 style="color: #1a73e8; font-weight: bold; margin: 25px 0 20px 0; font-size: 1.8em; text-align: center; padding-bottom: 12px;">${topic}</h3>
...
export const createBlogPrompt = (
  ...
  const basePrompt = `다음 요구사항에 따라 SEO 최적화된 블로그 글을 작성해주세요:
...
5. **검색 트렌드 고려**: 최신 검색 트렌드와 사용자 관심사를 반영한 소제목과 함께, 각 섹션별로 약 300-500자의 풍부한 설명을 작성해주세요.
* 설명은 단순 요약이 아닌, 다음 요소를 반드시 포함해야 합니다:
   - 사례 또는 통계 기반 인용
   - 독자 입장에서 느끼는 감정 또는 공감 지점
   - 전문가 또는 실사용자의 조언 또는 팁
   - 블로그 방문자가 '찾고 싶던 정보가 여기 다 있다'고 느낄 만큼 깊이 있고 실용적이어야 합니다
   - 검색 목적에 부합하는 완결형 콘텐츠처럼 느껴지도록 구성해주세요
   - 각 섹션은 공백 포함 300자에서 500자 사이로 작성해주세요
   - 설명이 140자를 초과하면 두 번째 마침표 이후 줄바꿈 및 공백 줄 추가
   - 각 문단은 <p> 태그로 감싸고, <p> 태그 사이에는 줄바꿈을 추가해야 합니다
   - 소제목과 본문은 모두 왼쪽 정렬(text-align: left)을 적용해야 합니다
6. 픽사베이 이미지 5개를 적절히 배치 (관련 키워드로)
7. 글 하단에 관련 태그 5-7개를 중앙 정렬로 배치
...
2. 시각적 요약 카드
3. 외부 링크 HTML
4. 도입부 (문제 제기 또는 흥미 유발) 글자수 150자 이내 작성
...
**3️⃣ 섹션별 글자수 제한 (엄격 준수) - 관리자 요청 반영**
- **각 H2 섹션 본문: 정확히 300-500자 범위 내**
- **500자 초과 금지, 300자 미만 금지** 
- **글자수 카운트 후 범위 확인 필수**
...
**🚨 글자수 제한 - 관리자 요청 반영 (절대 준수) 🚨**
**각 H2 섹션의 본문은 반드시 공백 포함 300자에서 500자 사이로 작성해야 합니다.**
- 이 글자수 제한은 절대적이며, 500자를 초과하거나 300자 미만이 되어서는 안 됩니다
- **140자를 초과하면 두 번째 문장의 마침표(.) 부분에서 반드시 줄바꿈을 하고 공백 줄 하나를 추가하세요**
- **모든 문단은 반드시 <p> 태그로 감싸서 작성하세요**
- **각 <p> 태그 사이에는 공백 줄바꿈을 추가하여 가독성을 높이세요**
- 섹션 작성 후 반드시 공백 포함 글자수를 카운트하여 300-500자 범위 내인지 확인하세요
...
**🚨 외부 참조 링크 스타일 적용 🚨**
${referenceLink ? `- 글 하단에 다음과 같은 스타일로 참조 링크를 포함하고 바로 아래에 태그 7개를 추가하세요:
...
**🚨 외부 참조 링크 및 문장 필수 적용 🚨**
${referenceSentence ? `**참조 문장 적용**: 제공된 참조 문장 "${referenceSentence}"을 글의 맥락에 맞게 자연스럽게 포함하세요.` : ''}
...
- **가독성 최우선**: 공백 포함 140자 도달 시 두 번째 문장 마침표에서 </p> 태그로 닫고 공백 줄바꿈 추가 후 새로운 <p> 태그로 시작
...
- **각 섹션은 정확히 공백 포함 300자에서 500자 사이의 적절한 분량이어야 합니다**
- **절대로 500자를 초과하거나 300자 미만이 되어서는 안 됩니다**
...
3. **SEO 최적화**: 검색에 최적화된 제목

지금 즉시 위 원칙에 따라 ${count}개의 제목을 생성해주세요.`;
  }
};
