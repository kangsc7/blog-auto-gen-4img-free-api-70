...
export const useGoogleTrends = () => {
  ...
  const getTrendingKeywords = async (): Promise<string[]> => {
    try {
      ...
      if (trendingTerms.length === 0) {
        // API 실패 시 연도 없는 최신 이슈 백업 키워드 목록
        return [
          '정부지원금 신청',
          '기초연금 인상',
          '청년도약계좌',
      ...
      // 모든 연도와 "년" 단어 필터링
      const filteredKeywords = trendingTerms.filter((keyword: string) => {
        return !keyword.includes('2023') && 
  ...
  const getRelatedKeywords = async (baseKeyword: string): Promise<string[]> => {
    try {
      ...
      // 관련 키워드에서도 모든 연도와 "년" 단어 필터링
      relatedQueries = relatedQueries.filter((keyword: string) => {
        return !keyword.includes('2023') && 
               !keyword.includes('2024') && 
               !keyword.includes('2025') && 
               !keyword.includes('2026') && 
